...
## Merge in new mito score for calculation of mMSS
new_mito_score = fread('new_mito_score_m_05_030524.txt')
allhetnew = merge(allhet,new_mito_score,by='unique',all.x=TRUE)

## Calculate new mMSS score
persamplecountn = allhetnew %>% group_by(SAMPLE) %>%
  summarize(
    mMSS = sum(mito_lc_score_m[AF != 1])) %>%
  as.data.frame()

## Save
write.table(persamplecountn, file='per_position_mMSS.txt', sep="\t", row.names=FALSE, quote = FALSE)
